...
import { isOddNumber, square, add, prop, propEq, equals } from "./utils";
...
describe("go 함수는", () => {
  it("초기값과 여러 함수를 인자로 받아서 합성한 함수에 초기값을 인자로 전달하여 즉시 실행한다", () => {
    const res = go(products, filter(propEq("category", "phone")), map(prop("price")), map(a => a * 0.8), reduce(add));
...
describe("curry 함수는", () => {
  ...
  it("두 번째 인자로 커링할 인자의 개수를 지정할 수 있다", () => {
    const add5 = curry((a, b, c, d, e = 100) => a + b + c + d + e, 3);
...
describe("L.flat 함수는", () => {
  it("iterable인 element를 펼쳐서 순회할 수 있는 generator객체를 반환한다", () => {
    expect([...L.flat(iter2)]).to.eql([1, ["a", 2], ["b", 3], 4, 5, 6, 7, [8, 9], 10]);
  });
...
describe("flat 함수는", () => {
  it("L.flat을 모두 즉시 평가한 결과를 배열에 담아 반환한다", () => {
    expect(flat(iter2)).to.eql([1, ["a", 2], ["b", 3], 4, 5, 6, 7, [8, 9], 10]);
  });
  it("string은 flat할 대상이 아닌 하나의 값으로 처리한다", () => {
...
describe("L.deepFlat 함수는", () => {
  it("중첩된 iterable을 모두 펼쳐서 순회하는 generator객체를 반환한다", () => {
    expect([...L.deepFlat(iter2)]).to.eql([1, "a", 2, "b", 3, 4, 5, 6, 7, 8, 9, 10]);
  });
...
describe("deepFlat 함수는", () => {
  it("L.deepFlat을 모두 즉시 평가한 결과를 배열에 담아 반환한다", () => {
    expect(deepFlat(iter2)).to.eql([1, "a", 2, "b", 3, 4, 5, 6, 7, 8, 9, 10]);
    expect(deepFlat([1, [2, [3, [4, [5], 6], 7], 8], 9])).to.eql([1, 2, 3, 4, 5, 6, 7, 8, 9]);
  });
...
describe("L.flatMap 함수는", () => {
  it("보조함수와 flat함수를 합성한 함수로 element를 평가하며 순회하는 generator객체를 반환한다", () => {
    expect([...L.flatMap(a => [a, a + 1, a + 2], iter)]).to.eql([1, 2, 3, 2, 3, 4, 3, 4, 5, 4, 5, 6, 5, 6, 7]);

    expect(take(8, L.flatMap(L.range, iter))).to.eql([0, 0, 1, 0, 1, 2, 0, 1]);
  });
});

describe("flatMap 함수는", () => {
  it("L.flatMap을 모두 즉시 평가한 결과를 배열에 담아 반환한다", () => {
    expect(flatMap(a => [a, a + 1, a + 2], iter)).to.eql([1, 2, 3, 2, 3, 4, 3, 4, 5, 4, 5, 6, 5, 6, 7]);
    expect(take(8, flatMap(L.range, iter))).to.eql([0, 0, 1, 0, 1, 2, 0, 1]);
  });
});
